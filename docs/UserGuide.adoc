...
* In v 1.3: Introduce calander for meetings. Able to detect a duplicate based on a phone number. Able to fully authenticate linkedIn, and share posts to LinkedIn. All alias commands will be completed. Auto-Complete of commands enabled.

* In v 1.4: Able to book meetings using the calender. Able to detect a duplicate based on a email address. Able to list out all Leads at user's request. CRM Book will alert you if a contact switches companies. At least two alias commands will be completed. Able to change theme statically.
...
Shows a list of all Leads in the CRM. +
Format: `disp Lead`, `display Lead`

****
* The search is case insensitive. e.g `lead` will match `Lead`
* Only a full word will be matched e.g. `Lead` will not match `Leads`
****
...
e.g. `d 5`, `delete 3`
* *Display* : `disp Lead`, `display Lead` +
e.g. `disp Lead`, `display Lead`
* *Exit* : `ex`, `exit`
