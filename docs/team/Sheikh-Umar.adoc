...
** Highlights: This enhancement affects existing commands and commands to be added in future, and certain commands may share the same alias. For example, the sort and select commands may share the alias command 's'.
Hence, for each command, a unique alias is used.
