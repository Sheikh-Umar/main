...
==== Current Implementation
The CRM-Book will ensure that non-duplicates. +
...
The phone number and email address of a Lead to be added will be checked will all Leads and Contacts stored in a CRM-Book.
If any of the Leads/Contacts in the CRM-Book has either the phone number or email address of a Lead to be added, the CRM application will display a message that the lead is a duplicate and will reject this lead.
...
* **Alternative 1 (current choice):** Iterate through the list that stores all Leads and Contacts and identify a Lead/Contact with the same phone number or email address as a
Lead to be added.
** Pros: Identify a duplicate in linear time.
...
Umar: +
**Major feature** : Ensuring non-duplicate data is retrievable instantly. +

* In the sales industry, duplicates come in the form of different ordering of name and unique addresses.
However, key information such as the phone number and email address will remain the same in these duplicates.
Hence, the CRM aims to detect a duplicate based on this information.
However, the lead will have the same phone number or email address. Hence, the CRM aims to tackle this issue
* Furthermore, a salesperson may want to view a list of either Leads or Contacts only rather than a list of all List and Contacts.
Hence, the CRM aims to assist salespeople who want to keep track of the Leads and Contacts stored.
...
|`* * *` |salesperson |identify a duplicate lead based on email address|not have redundant data in my CRM

|`* * *` |salesperson |identify a duplicate lead based on phone number|not have redundant data in my CRM

|`* * *` |salesperson |identify a duplicate lead based on reverse ordering of name|not have redundant data in my CRM
