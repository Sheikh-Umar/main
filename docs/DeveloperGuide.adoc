...
A user is not able to assign a remark to a person when adding the person. Therefore, all people in the CRM is originally without any remark by default.
...
A string array is declared in PersonCard.java to include all the different colors for tags. These colors are defined, with their backgroud color and text color spelled out, in the the DarkTheme.css file.
...
To implement a CRM, Leads and Contacts must be differentiated so that they can each hold different fields like a proper CRM. To do so, they currently extend the Person class so that all existing functionality associated with Persons will work with both Leads an Contacts. Unique fields will be given their own classes like the original generic fields like Name and Address.

In a CRM, users start by adding Leads, who are people they have not sold to yet. As they begin selling, the people who they have sold to become Contacts.

Many existing commands like AddCommand have been modified to fit the new paradigm so that it only adds Leads.

A new conversion command ConvertCommand has been added to convert Leads into Contacts
...
** Cons: Declarations of Person variables may confuse new developers even as a general class for Leads an Contacts.
...
After the CRM Book has received the authorization code and the app secret from the decrypter, they are sent back to LinkedIn to request for an AccessToken. Once the AccessToken has been received, the user is considered to be successfully logged in, and the accessToken can be used by the CRM Book to make requests to LinkedIn on behalf of the user.
...
=== Identifying a Duplicate Lead feature

==== Current Implementation
The CRM application will ensure that every lead in the address book is unique. +

The feature is implemented by checking the phone number and email address of a new lead with all of the phone numbers and email addresses of all leads and contacts. +

The feature will be implemented on the add command because the current application detects duplicates at the add command. +

The phone number and email address of a lead will be through the list of Leads and Contacts.
If either the phone number or the email address of a Lead to be added is found,
...
===== How to implement identifying a duplicate
* **Alternative 1 (current choice):** Iterate through the list of Leads and Contacts.
...
Umar: +
**Major feature** : Duplicate Detection

* Identifying a duplicate. +
* In the sales industry, the same lead may be stored in the CRM application in different forms but retains the same information.
(Example: A Lead already in the CRM application is about to be entered into the CRM application
in the form of last name-first name. The CRM application will not detect this duplicate
and successfully store this lead. Now the CRM application has a duplicate.)
...
* Allows a Salesperson to import a CSV file into the CRM +
* Allows a Salesperson to import a large number of people into the CRM in one time.


**Major Feature** : Remark Command

* Allows a Salesperson to add/remove/edit a remark of a person in the CRM +
...
|`* * *` |salesperson |identify a duplicate Lead or Contact|not have redundant data in my CRM
...
|`*` |salesperson |sort Leads and Contacts list based on importance|identify priority Leads and Contacts
...
=== Use case: Detect a duplicate Lead or Contact

*MSS*

1. User enters a Lead to add
2. CRM Book determines if Lead can be added
3. CRM Book displays message on whether Lead entered can be added or is a duplicate
+
Use case ends.
...
[none]
* 1a. User either entered Lead or add command incorrectly.
+
[none]
** 1a1. CRM displays error message.
+
Use case ends.

* 2a. Lead entered has a phone number or email address stored earlier.
** 2a1. CRM displays message that a possible duplicate detected.
+
Use case ends.

* 2b. Lead entered does not have a phone number or email address stored earlier.
** 2b1. CRM adds Lead.
** 2b2. CRM displays message of successful addition of Lead.
+
Use case ends.

=== Use case: User seeks list of either Leads or Contacts

*MSS*

1. User enters command for list of either Lead or Contact.
2. CRM retrieves Leads or Contacts.
3. CRM displays list of Leads or Contacts.
...
Use case ends.
+
* 2a. Desired list is empty.
** 2a1. CRM displays message that list is empty.
+
Use case ends.
